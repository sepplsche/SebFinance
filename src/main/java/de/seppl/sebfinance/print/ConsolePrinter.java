...
public class ConsolePrinter<T> {
	...
	private LinkedList<Column> line(Collection<PrintableColumn<T>> columns, T element, Collection<T> elements) {
	...
	private void printLine(LinkedList<Column> line) {
		...
		line.stream().forEach(column -> System.out.print(printColumn(column, lastColumn.equals(column))));
		System.out.println();
	}
	...
	private String printColumn(Column column, boolean lastColumn) {
		return "|" + (column.left ? printLeftBound(column) : printRightBound(column)) + (lastColumn ? "|" : "");
	}
